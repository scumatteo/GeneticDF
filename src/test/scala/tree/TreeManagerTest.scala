...
class TreeManagerTest extends AnyFunSuite {
  ...
  test(s"The method createInitialPopulation should return a list of IE-Tree of size = populationSize"){
    assert(TreeManager.createInitialPopulation(populationSize).size == populationSize)
  }

  test(s"The method createRandomTree should create a random IE-Tree of depth <= maxDepth"){
